...
int voxel_upsampler(int M, int R1, int R2, int R3, int nsample, float radius, 
    ...
    const float *voxel_score = voxel_score_tensor.data<float>();
    float *pt_score = pt_score_tensor.data<float>();
    float *distance = distance_tensor.data<float>();

    voxel_upsampler_kernel_launcher(M, R1, R2, R3, nsample, radius, z_range, y_range, x_range, new_xyz, xyz,
                                    new_coords, point_indices, voxel_score, pt_score, distance);
    return 1;
}
